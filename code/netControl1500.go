...
func findGethPID() (int, error) {
	cmd := exec.Command("sh", "-c", "ps -eo pid,comm | grep geth | grep -v grep")
	...
	if len(pids) == 0 {
		return 0, fmt.Errorf("No geth process found")
	}

	pidFields := strings.Fields(pids[0])
	if len(pidFields) == 0 {
		return 0, fmt.Errorf("Invalid process format")
	}
